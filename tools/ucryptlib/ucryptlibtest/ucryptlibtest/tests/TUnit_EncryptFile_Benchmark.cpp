...
void TUnit_EncryptFile_Benchmark()
{
	std::string path = "tests/encryption_tests/";
	std::string fileName = "benchmark_file";
	...
	size_t fileContentRandomSize = 100000000; // x*10^6 Bytes = x MB
	uint8_t *fileContentRandom = new uint8_t[fileContentRandomSize];
	randombytes_buf(&fileContentRandom[0], fileContentRandomSize);

	uCrypt::uCryptLib::blake2s_Hash(hashOriginalFile, &fileContentRandom[0], NULL, szOutBytes, fileContentRandomSize, 0);
	...
	if(originalFileStream.is_open())
		originalFileStream.write((char*)&fileContentRandom[0], fileContentRandomSize);
	else
	...
	remove((path + fileName).c_str());			// Original File: delete file if it exists already
	remove((path + fileName + ".encrypted").c_str());			// Encrypted File: delete file if it exists already

	if( 0 == memcmp(hashOriginalFile, hashDecryptedFile, sizeof(hashOriginalFile)/sizeof(uint8_t)) )
	{
		printf("TUnit_EncryptFile_Benchmark: OK\n");
	}
	else
	{
		printf("TUnit_EncryptFile_Benchmark: FAILED\n");
