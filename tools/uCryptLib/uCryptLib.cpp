...
namespace uCrypt
{
  ...
  const char uCryptLib::b58digits_ordered[] =
  ...
  uCryptLib::uCryptLib(std::string eMail, std::string passwd)
  {
    ...
    scrypt(hash, BLAKE2S_OUTBYTES, salt, salt_len,
           _SCRYPT_N, _SCRYPT_r, _SCRYPT_p, _secretKey, crypto_box_PUBLICKEYBYTES);
    crypto_scalarmult_curve25519_base(_publicKey, _secretKey);

    blake2s_Hash(identificationNumberHash, _publicKey, NULL, _ID_CHECKSUM_LEN,
    ...
    identificationNumber[(_ID_CHECKSUM_LEN + crypto_box_PUBLICKEYBYTES) - 1] =
        identificationNumberHash[0];

    size_t encB58Len = sizeof(encIdentificationNumber) / sizeof(char);
    size_t *encB58sz = &encB58Len;

    base58_Encode(encIdentificationNumber, encB58sz, identificationNumber,
                  _ID_CHECKSUM_LEN + crypto_box_PUBLICKEYBYTES);
  ...
  uint32_t uCryptLib::readChunkLength(uint8_t *data, size_t szData, size_t offset)
  {
    ...
    ret = ((data[offset+3] << 24 ) | (data[offset+2] << 16 ) |
  ...
  uint8_t uCryptLib::DecryptFile(std::string fileName, std::string sourcePath, std::string targetPath)
  {
    if (!isInitialized)
    {
    ...
    else
    {
      ...
      const uint32_t headerLength =
          readChunkLength(&vOriginalFileContent[0], vOriginalFileContent.size(), _MAGIC_BYTES_SIZE);
      ...
      std::string strJsonHeader =
          strOriginalFileContent.substr(_MAGIC_BYTES_SIZE + _CHUNK_LEN_ARR_LEN,
                                        _MAGIC_BYTES_SIZE + _CHUNK_LEN_ARR_LEN +
                                        headerLength);
      std::vector<uint8_t> encryptedFileContent(vOriginalFileContent.begin() +
                                                startOfCiphertext,
                                                vOriginalFileContent.end());
      ...
      else
      {
        ...
        { // the fuck ? oO checkme
          ...
          for (rapidjson::Value::ConstMemberIterator itr = decryptInfoJsonObject.MemberBegin();
               itr != decryptInfoJsonObject.MemberEnd(); ++itr)
          {
        ...
        for (size_t i=0; i < decryptInfoCount; ++i) //TODO anwait
        {
        ...
        strJsonDecryptInfo =
            strJsonDecryptInfo.substr(strJsonDecryptInfo.find("{", 0),
                                      (strJsonDecryptInfo.find("}", 0) + 1) -
                                      strJsonDecryptInfo.find("{", 0));
        ...
        else
        {
          ...
          std::vector<uint8_t> vEncryptedFileInfo(strEncryptedFileInfo.begin(),
                                                  strEncryptedFileInfo.end());

          uint8_t *decrypted_JsonFileInfo =
              new uint8_t[vEncryptedFileInfo.size() - crypto_box_MACBYTES];
          if (0 != crypto_box_open_easy(decrypted_JsonFileInfo,
                                       &vEncryptedFileInfo[0],
                                       vEncryptedFileInfo.size(),
                                       &vUniqueNonce[0],
                                       _publicKey,
                                       _secretKey))
          {
          ...
          std::string strJsonFileInfo(reinterpret_cast <char const*> (decrypted_JsonFileInfo),
                                      vEncryptedFileInfo.size());

          delete[] decrypted_JsonFileInfo;

          strJsonFileInfo = strJsonFileInfo.substr(strJsonFileInfo.find("{", 0),
                                                   (strJsonFileInfo.find("}", 0) + 1) -
                                                   strJsonFileInfo.find("{", 0));
          ...
          else
          {
            ...
            chunkSizeDecrypted = readChunkLength(&encryptedFileContent[0], encryptedFileContentSize, 0 /*offset=0*/); //todo anwait checkme
            chunkSizeEncrypted = chunkSizeDecrypted + _CHUNK_LEN_ARR_LEN + _PADDING_BYTES_LEN;
            std::vector<uint8_t> encryptedChunk(encryptedFileContent.begin() + _CHUNK_LEN_ARR_LEN,
                                                encryptedFileContent.begin() + chunkSizeEncrypted);
            ...
            if (0 != crypto_secretbox_open_easy(decryptedFileName,
                                                &encryptedChunk[0],
                                                encryptedChunk.size(),
                                                &vFullNonce[0],
                                                &vFileKey[0]))
            ...
            std::string strDecryptedFileName(reinterpret_cast <char const*> (decryptedFileName),
                                             chunkSizeDecrypted);
            std::string saveFileName = targetPath + strDecryptedFileName;
            ...
            for (size_t i = 1; i <= (cTotalChunks); ++i)
            {
              ...
              std::vector<uint8_t> encryptedChunk(encryptedFileContent.begin() +
                                                  itDataPosition +
                                                  _CHUNK_LEN_ARR_LEN,
                                                  encryptedFileContent.begin() +
                                                  itDataPosition +
                                                  chunkSizeEncrypted);

              uint8_t *decryptedFileContent = new uint8_t[chunkSizeDecrypted];
              if ( 0 != crypto_secretbox_open_easy(decryptedFileContent,
                                                  &encryptedChunk[0],
                                                  encryptedChunk.size(),
                                                  &vFullNonce[0],
                                                  &vFileKey[0]))
  ...
  uint8_t uCryptLib::EncryptFile(std::string fileName,
                                 std::string sourcePath,
                                 std::string recipientIDs[],
                                 uint8_t numberRecipients,
                                 std::__cxx11::string targetPath)
  {
    if (!isInitialized)
    {
      return 0xff; // Uninitialized Error
    ...
    std::string saveFileName = targetPath + fileName + ".encrypted";
    std::string fileNameInclPath = sourcePath + fileName;
    ...
    encryptedFileContent = encryptedFileContent +
                           std::string(vChunkLenArray.begin(),
                                       vChunkLenArray.end()) +
    ...
    else
    {
      ...
      for (size_t i = 0; i < cTotalChunks; ++i)
      {
        ...
        std::vector<uint8_t> decryptedChunk(vDecryptedFileContent.begin() +
                                            itDataPosition,
                                            vDecryptedFileContent.begin() +
                                            itDataPosition +
                                            chunkSizeDecrypted);

        uint8_t *encryptedChunk = new uint8_t[chunkSizeEncrypted];

        if (0 != crypto_secretbox_easy(encryptedChunk,
                                       &decryptedChunk[0],
                                       decryptedChunk.size(),
                                       &vFullNonce[0],
                                       &vFileKey[0]))
    ...
    // Calculate Hash
    uint8_t calculatedFileContentHash[BLAKE2S_OUTBYTES];
    blake2s_Hash(calculatedFileContentHash,
                 encryptedFileContent.c_str(),
                 NULL,
                 BLAKE2S_OUTBYTES,
                 encryptedFileContent.size(),
                 0);
    ...
    std::string json_Object = "{\"version\":1,\"ephemeral\":\"" +
                              base64_Encode(senderEphemeralPublic, _PUBLICKEYBYTES) +
                              "\",";
    ...
    {
      std::string strJsonFileInfo = "{\"fileKey\":\"" +
                                    base64_fileKey +
                                    "\",\"fileNonce\":\"" +
                                    base64_fileNonce +
                                    "\",\"fileHash\":\"" +
      ...
      base58_Decode(decodedRecipientPublic,
                    &actPos,
                    recipientIDs[i].c_str(),
                    recipientIDs[i].length());
      ...
      if (0 != crypto_box_easy(encrypted_jsonFileInfo,
                               pJsonFileInfo,
                               jsonFileInfo_Len,
                               uniqueNonce,
                               recipientPublic,
      ...
      std::string strJsonDecryptInfo_N = "{\"senderID\":\"" +
      ...
      if (0 != crypto_box_easy(encrypted_jsonDecryptInfo_N,
                               pJsonDecryptInfo_N,
                               jsonDecryptInfo_N_Len,
                               uniqueNonce, recipientPublic,
                               senderEphemeralSecret))
      {
        return 1; // General Encrytion Failure
      }

      std::string base64_decryptInfo_N = base64_Encode(encrypted_jsonDecryptInfo_N,
                                                       encrypted_jsonDecryptInfo_N_Len);
      base64_decryptInfo = base64_decryptInfo + "\"" +
                           base64_uniqueNonce + "\":\"" +
                           base64_decryptInfo_N + "\",";
    }

    base64_decryptInfo = base64_decryptInfo.substr(0,
                                                   base64_decryptInfo.size() - 1) +
                         "}}"; ///< remove last "," and add "}}"
    ...
    encryptedFile =
        std::string(vMagicBytes,
                    vMagicBytes +
                    (sizeof(vMagicBytes) / (sizeof(vMagicBytes[0])))) +
        std::string(vChunkLenArray.begin(), vChunkLenArray.end()) +
        json_Object;
    ...
    encryptedFileStream.open(saveFileName,
                             std::ios::binary |
                             std::ios::out |
                             std::ios_base::app);
  ...
  double uCryptLib::getBitEntropy(std::string s)
  {
    ...
    /*
    for_each (char c in s)  // Anwendung nicht empfohlen https://msdn.microsoft.com/de-de/library/ms177202.aspx
    {
      frequencies[ c ] ++ ;
    }
    */
    // reimplementation of the shit above
    for (char c : s) //todo anwait: check if correct
    {
      frequencies[c]++;
    }
  ...
  std::string uCryptLib::base64_Encode(unsigned char const* bytes_to_encode, unsigned int in_len)
  {
    ...
    while (in_len--) // original in_len--
    {
      char_array_3[i++] = *(bytes_to_encode++); // original i++ bytes_to_encode++
      if (i == 3)
      {
    ...
    if (i)
    {
      ...
      for (j = 0; (j < i + 1); j++) // original j++
      {
        ret += base64_chars[char_array_4[j]];
      }

      while((i++ < 3)) // original ((i++ < 3))
      {
        ret += '=';
      }
  ...
  int uCryptLib::blake2s_Hash(uint8_t *out,
                              const void *in,
                              const void *key,
  ...
  bool uCryptLib::base58_Encode(char *b58, size_t *b58sz, const void *data, size_t binsz)
  {
    ...
    while (zcount < static_cast <ssize_t> (binsz) && !bin[zcount])
    {
      ++zcount;
    }
    ...
    for (i = zcount, high = size - 1; i < static_cast <ssize_t> (binsz); ++i, high = j)
    {
    ...
    for (j = 0; j < static_cast <ssize_t> (size) && !buf[j]; ++j); // empty for loop
    ...
    for (i = zcount; j < static_cast <ssize_t> (size); ++i, ++j)
    {
